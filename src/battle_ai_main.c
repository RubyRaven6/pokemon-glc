...
// the following checks apply to any target (including user)
    ...
    case EFFECT_STEELSURGE:
        if (IsHazardOnSide(GetBattlerSide(battlerDef), HAZARDS_STEELSURGE)
          || PartnerMoveIsSameNoTarget(GetPartnerBattler(battlerAtk), move, aiData->partnerMove)) //Only one mon needs to set up Steelsurge
            ADJUST_SCORE(-10);
        break;
    case EFFECT_ICE_RINK:
        if (IsHazardOnSide(GetBattlerSide(battlerDef), HAZARDS_ICE_RINK)
          || PartnerMoveIsSameNoTarget(GetPartnerBattler(battlerAtk), move, aiData->partnerMove)) // Only one mon needs to set up Ice Rink
            ADJUST_SCORE(-10);
        break;
    ...
    for (u32 effectId = 0; effectId < additionalEffectCount; effectId++)
    {
        ...
        if (!MoveEffectIsGuaranteed(battlerAtk, aiData->abilities[battlerAtk], additionalEffect))
        {
            if (additionalEffect->self
             || IsAdditionalEffectBlocked(battlerAtk, aiData->abilities[battlerAtk], battlerDef, aiData->abilities[battlerDef], move))
                continue;
            ...
            case MOVE_EFFECT_OVEREXPOSURE:
                if (!GetBattlerPartyState(battlerDef)->overexposed
                 && (HasMoveWithType(battlerAtk, TYPE_ICE)
                  || HasMoveWithType(GetPartnerBattler(battlerAtk), TYPE_ICE)))
                    ADJUST_SCORE(DECENT_EFFECT);
                break;
