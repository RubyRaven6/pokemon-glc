...
{
    u32 i;
    u32 weight = GetBattlerWeight(ctx->battlerDef, ctx->abilities[ctx->battlerDef], ctx->holdEffects[ctx->battlerDef]);
    u32 basePower = 120;
...
bool32 CanMoveSkipAccuracyCalc(struct BattleCalcValues *cv, u32 weather, enum ResultOption option)
{
    ...
    else
    {
        ...
        if ((attackerWeather & B_WEATHER_RAIN) && MoveAlwaysHitsInRain(cv->move))
            effect = TRUE;
        else if ((attackerWeather & B_WEATHER_SUN) && MoveAlwaysHitsInSun(cv->move))
            effect = TRUE;
