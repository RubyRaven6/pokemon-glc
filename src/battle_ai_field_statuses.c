...
bool32 WeatherChecker(enum BattlerId battler, u32 weather, enum FieldEffectOutcome desiredResult)
{
    ...
    for (u32 battlerIndex = 0; battlerIndex < gBattlersCount; battlerIndex++)
    {
        ...
        if (weather & B_WEATHER_RAIN)
            ...
            result[sideBattlers] = BenefitsFromWindstorm(battler);
...
static enum FieldEffectOutcome BenefitsFromSun(enum BattlerId battler)
{
    ...
    if (HasMoveWithFlag(GetBattlerLeftFoe(battler), MoveAlwaysHitsInSun))
        return FIELD_EFFECT_NEGATIVE;
...
static enum FieldEffectOutcome BenefitsFromWindstorm(enum BattlerId battler)
{
    ...
    bool32 benefitsPartner = HasPartner(battler)
                          && (HasDamagingMoveOfType(GetPartnerBattler(battler), TYPE_FLYING)
                           || IS_BATTLER_OF_TYPE(GetPartnerBattler(battler), TYPE_FLYING));
    bool32 benefitsFoe = HasDamagingMoveOfType(GetBattlerLeftFoe(battler), TYPE_FLYING)
                      || IS_BATTLER_OF_TYPE(GetBattlerLeftFoe(battler), TYPE_FLYING)
                      || HasDamagingMoveOfType(GetBattlerRightFoe(battler), TYPE_FLYING)
                      || IS_BATTLER_OF_TYPE(GetBattlerRightFoe(battler), TYPE_FLYING);
...
s32 CalcWeatherScore(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Move move, struct AiLogicData *aiData)
{
    ...
    case BATTLE_WEATHER_SUN:
        if (ShouldSetWeather(battlerAtk, B_WEATHER_SUN))
        {
            ...
            if (HasMoveWithFlag(battlerDef, MoveHas50AccuracyInSun) || HasMoveWithFlag(GetPartnerBattler(battlerDef), MoveHas50AccuracyInSun))
                score += WEAK_EFFECT;
            if (HasMoveWithFlag(battlerAtk, MoveAlwaysHitsInSun) || HasMoveWithFlag(GetPartnerBattler(battlerAtk), MoveAlwaysHitsInSun))
                score += WEAK_EFFECT;
        }
        break;
    ...
    case BATTLE_WEATHER_WINDSTORM:
        if (ShouldSetWeather(battlerAtk, B_WEATHER_WINDSTORM))
        {
            ...
            if (HasDamagingMoveOfType(battlerAtk, TYPE_FLYING) || HasDamagingMoveOfType(GetPartnerBattler(battlerAtk), TYPE_FLYING))
                score += WEAK_EFFECT;
            if (IS_BATTLER_OF_TYPE(battlerAtk, TYPE_FLYING) || IS_BATTLER_OF_TYPE(GetPartnerBattler(battlerAtk), TYPE_FLYING))
                score += WEAK_EFFECT;
            if (HasDamagingMoveOfType(battlerDef, TYPE_FLYING) || HasDamagingMoveOfType(GetPartnerBattler(battlerDef), TYPE_FLYING))
                score -= WEAK_EFFECT;
        }
    default:
        break;
