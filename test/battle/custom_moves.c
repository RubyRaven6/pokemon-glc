...
ASSUMPTIONS
{
    ...
    ASSUME(GetMoveAccuracy(MOVE_PSYCHE_LOCK) == 75);
    ...
    ASSUME(GetMovePower(MOVE_RAZZLE_DAZZLE) == 50);
    ...
    ASSUME(GetMoveEffect(MOVE_ORBITAL_PULL) == EFFECT_ORBITAL_PULL);
    ASSUME(GetMoveType(MOVE_ORBITAL_PULL) == TYPE_GROUND);
    ASSUME(GetMoveCategory(MOVE_ORBITAL_PULL) == DAMAGE_CATEGORY_STATUS);
    ...
    ASSUME(GetMoveEffect(MOVE_SYNCHROSWIM) == EFFECT_SYNCHROSWIM);
...
SINGLE_BATTLE_TEST("Custom Moves - Orbital Pull charges then boosts offenses and sets gravity")
{
    GIVEN {
        ...
        TURN { MOVE(player, MOVE_ORBITAL_PULL); MOVE(opponent, MOVE_CELEBRATE); }
